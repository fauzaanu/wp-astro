...
export async function getPostsByCategory(categoryId, page = 1, perPage = 12) {
    ...
    const posts = await fetchAPI(`posts?categories=${categoryId}&per_page=${perPage}&page=${page}&_embed`);
    const total = parseInt(posts.headers?.get('X-WP-Total') || '0');
...
export async function searchPosts(query, page = 1, perPage = 12) {
    ...
    const posts = await fetchAPI(`posts?search=${encodeURIComponent(query)}&page=${page}&per_page=${perPage}&_embed`);
    const total = parseInt(posts.headers?.get('X-WP-Total') || '0');
    const totalPages = parseInt(posts.headers?.get('X-WP-TotalPages') || '1');
